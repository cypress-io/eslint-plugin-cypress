...
const TARGET_CALLEE_NAMES = ['it', 'specify', 'context', 'describe']
...
module.exports = {
  ...
  create(context) {
    function isTargetTestCallee(callee) {
      // Matches: it(), specify(), context(), describe(), and their .only/.skip variants
      if (callee.type === 'Identifier') {
        return TARGET_CALLEE_NAMES.includes(callee.name)
      }
      if (callee.type === 'MemberExpression'
        && !callee.computed
        && callee.object.type === 'Identifier'
        && TARGET_CALLEE_NAMES.includes(callee.object.name)
