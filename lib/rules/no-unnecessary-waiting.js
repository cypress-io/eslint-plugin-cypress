...
function isIdentifierNumberConstArgument (node, scope) {
  ...
  /* const amount = 1000, const amount = '@alias',
     const amount = someFunction() or const amount = require("./my-contants")
     cy.wait(amount)
  */
  if (isVariable) {
    if (!definition.node.init) return false

    return definition.node.init.type === 'CallExpression'
      || typeof definition.node.init.value === 'number'
  }
